...
import { Product, Order, AdminAnalytics, StudioSettings, ColorVariant } from '../types';
import { generateAutoColorways, ARTISAN_PRESET_COLORWAYS } from '../utils/colorUtils';
import { compressImage, compressBase64Image } from '../utils/imageUtils';
...
export default function AdminPanel({ onProductUpdated, onSettingsUpdated }: AdminPanelProps) {
  ...
  const [formImagesArray, setFormImagesArray] = useState<string[]>([]);
  const [formColorsInput, setFormColorsInput] = useState<string>('');
  const [formColorVariants, setFormColorVariants] = useState<ColorVariant[]>([]);
  ...
  // New Promo Code Builder state
  const [newPromoCode, setNewPromoCode] = useState<string>('');
  const [newPromoType, setNewPromoType] = useState<'percent' | 'fixed' | 'free_shipping'>('percent');
  const [newPromoVal, setNewPromoVal] = useState<number>(10);
  const [newPromoMinOrder, setNewPromoMinOrder] = useState<number>(0);
  const [newPromoDesc, setNewPromoDesc] = useState<string>('');

  const handleAddPromoCode = () => {
    if (!settings || !newPromoCode.trim()) return;
    const cleanCode = newPromoCode.trim().toUpperCase();
    const existing = settings.adminPromoCodes || [];
    if (existing.some(c => c.code === cleanCode)) {
      alert(`Promo code "${cleanCode}" already exists.`);
      return;
    }

    const newEntry = {
      code: cleanCode,
      discountPercent: newPromoType === 'percent' ? newPromoVal : undefined,
      discountAmount: newPromoType === 'fixed' ? newPromoVal : undefined,
      waiveDelivery: newPromoType === 'free_shipping' ? true : undefined,
      minOrderValue: newPromoMinOrder > 0 ? newPromoMinOrder : undefined,
      description: newPromoDesc.trim() || `${cleanCode} Voucher Code`
    };

    setSettings({
      ...settings,
      adminPromoCodes: [...existing, newEntry]
    });
  ...
  const handleDeletePromoCode = (codeToDelete: string) => {
    ...
    setSettings({
      ...settings,
      adminPromoCodes: existing.filter(c => c.code !== codeToDelete)
    });
  };
  ...
  const [isCompressingStorage, setIsCompressingStorage] = useState<boolean>(false);

  const handleSmartCleanStorage = async () => {
    setIsCompressingStorage(true);
    try {
        ...
        currentProducts.map(async (product) => {
          ...
          if (updatedProduct.images && updatedProduct.images.length > 0) {
            const newUrls = await Promise.all(
              updatedProduct.images.map(async (url) => {
                if (url.startsWith('data:image/')) {
                  const newUrl = await compressBase64Image(url, 400, 0.4);
                  return newUrl;
                }
                return url;
              })
            ...
            // Check if any url actually changed size
            if (JSON.stringify(newUrls) !== JSON.stringify(updatedProduct.images)) {
              updatedProduct.images = newUrls;
              changed = true;
            }
          }
      ...
      localStorage.setItem('vlaksha_products', JSON.stringify(updatedProducts));
      setProducts(updatedProducts);
      setSyncSuccessMessage(`Smart Clean complete! Compressed ${compressedCount} products. Your storage has been optimized without losing any data.`);
  ...
  const handleExportProductsJSON = async () => {
    ...
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `vlaksha_crafts_products_backup_${new Date().toISOString().split('T')[0]}.json`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (Array.isArray(importedData)) {
          if (confirm(`Are you sure you want to import ${importedData.length} products? This will overwrite your existing products list.`)) {
            
            alert("Starting import and compression of images to prevent quota issues. This may take a few seconds.");
            
            // Compress on the fly during import
            const compressedData = await Promise.all(
              importedData.map(async (product: any) => {
                let p = { ...product };
                if (p.images && p.images.length > 0) {
                  p.images = await Promise.all(p.images.map(async (url: string) => {
                    if (url.startsWith('data:image/')) {
                      return await compressBase64Image(url, 400, 0.4);
                    }
                    return url;
                  }));
                }
            ...
            localStorage.setItem('vlaksha_products', JSON.stringify(compressedData));
            setProducts(compressedData);
            alert("Products successfully imported and optimized!");
            loadAdminData();
          }
        } else {
  ...
  const handleAddOrEditProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dbService.saveProduct(fullProduct);
  ...
  const startEditProduct = (p: Product) => {
    setEditingProduct(p);
    setFormImagesArray([...p.images]);
    setFormColorVariants(p.colors || []);
  ...
  const startNewProduct = () => {
    ...
    setFormImagesArray([]);
    const autoColors = generateAutoColorways('mandala-wall-plates');
    setFormColorVariants(autoColors);
    setFormColorsInput(autoColors.map(c => `${c.name}:${c.value}`).join(', '));
    ...
    <div id="admin-dashboard-container" className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[var(--theme-primary)]/15 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSmartCleanStorage}
            disabled={isCompressingStorage}
            className="px-3.5 py-2 rounded-none border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors text-xs font-semibold uppercase tracking-wider font-sans disabled:opacity-50"
          >
            {isCompressingStorage ? (
              <><Loader2 className="w-3.5 h-3.5 inline mr-1 animate-spin" /> Compressing...</>
            ) : (
              <><Sparkles className="w-3.5 h-3.5 inline mr-1 mb-0.5" /> Smart Clean Storage</>
            )}
          </button>
          <button
            onClick={() => {
              if (window.confirm('Are you sure you want to completely reset local storage? This will fix the QuotaExceededError by deleting all local products (unsynced).')) {
                localStorage.removeItem('vlaksha_products');
                window.location.reload();
              }
            }}
            className="px-3.5 py-2 rounded-none border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors text-xs font-semibold uppercase tracking-wider font-sans"
          >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="py-2.5 px-4 bg-stone-50 text-stone-700 border border-stone-200 font-sans text-xs font-semibold uppercase tracking-widest rounded-none hover:bg-stone-100 transition-all flex items-center gap-1.5 shadow-sm"
              ...
              <button
                ...
                className="py-2.5 px-4 bg-emerald-50 text-emerald-700 border border-emerald-200 font-sans text-xs font-semibold uppercase tracking-widest rounded-none hover:bg-emerald-100 transition-all flex items-center gap-1.5 shadow-sm"
                title="Download your entire product list to keep it safe"
              >
              ...
              <button
                onClick={() => {
                  if (confirm('EMERGENCY RESET: This will wipe your ENTIRE browser storage for this site (including products, orders, and login state) to fix quota errors. Do you want to continue?')) {
                    localStorage.clear();
                    location.reload();
                  }
                }}
                className="py-2.5 px-4 bg-red-50 text-red-600 border border-red-200 font-sans text-xs font-semibold uppercase tracking-widest rounded-none hover:bg-red-600 hover:text-white transition-all flex items-center gap-1.5 shadow-sm"
              >
                <AlertTriangle className="w-4 h-4" />
                <span>Reset Entire Storage</span>
              </button>
              <button
      ...
      {isEditingProduct && editingProduct && (
        ...
        >
          ...
          <div className="space-y-3 bg-[#fdfbf7] p-4 border border-[var(--theme-primary)]/20 rounded-none">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-stone-200/60 pb-2">
              ...
              <button
                type="button"
                onClick={() => {
                  setFormImagesArray([...formImagesArray, 'https://images.unsplash.com/photo-1579783902614-a3fb3927b6a5?q=80&w=600&auto=format&fit=crop']);
                }}
            ...
            <details className="text-[9px] font-sans text-stone-400 cursor-pointer">
              <summary>View raw line-separated image URLs list</summary>
              <textarea
                value={formImagesArray.join('\n')}
                onChange={(e) => setFormImagesArray(e.target.value.split('\n').map(s => s.trim()).filter(Boolean))}
          ...
          {/* Artisan Color Variants & Swatch Images Manager */}
          <div className="space-y-3 bg-[#fdfbf7] p-4 border border-[var(--theme-primary)]/20 rounded-none">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-stone-200/60 pb-2">
              <div>
                <label className="block text-[10px] font-sans uppercase tracking-widest font-extrabold text-[var(--theme-primary)] flex items-center gap-1.5">
                  <Palette className="w-3.5 h-3.5" />
                  Artisan Color Variants & Swatch Photos
                </label>
                <p className="text-[9px] text-stone-500 font-sans">
                  Upload different color craft photos or assign custom color hex codes. Customers can switch color swatches in the catalog & 3D viewer.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    const generated = generateAutoColorways(editingProduct.category);
                    setFormColorVariants(generated);
                    setFormColorsInput(generated.map(c => `${c.name}:${c.value}`).join(', '));
                  }}
                  className="py-1 px-2.5 bg-amber-600/10 hover:bg-amber-600 text-amber-900 hover:text-white transition-colors text-[9px] font-sans uppercase font-bold tracking-wider flex items-center gap-1 border border-amber-600/30"
                >
                  <Sparkles className="w-3 h-3 text-amber-600" />
                  Auto-Generate 5 Artisan Colorways
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    setFormColorVariants([
                      ...formColorVariants,
                      { name: 'New Artisan Color', value: '#1E3A8A', image: '' }
                    ]);
                  }}
                  className="py-1 px-2.5 bg-[var(--theme-primary)] text-white hover:bg-[var(--theme-accent)] transition-colors text-[9px] font-sans uppercase font-bold tracking-wider flex items-center gap-1"
                >
                  <Plus className="w-3 h-3" />
                  Add Color
                </button>
            ...
            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
              {formColorVariants.length === 0 ? (
                <div className="text-center py-6 border border-dashed border-stone-200 text-stone-400 text-xs font-sans">
                  No color variants created yet. Click "Auto-Generate 5 Artisan Colorways" to automatically build curated color options!
                </div>
              ) : (
                formColorVariants.map((variant, vIdx) => (
                  <div key={vIdx} className="p-3 bg-white border border-stone-200 shadow-2xs flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                    {/* Color Swatch Preview & Picker */}
                    <div className="flex items-center gap-2 shrink-0">
                      <input
                        type="color"
                        value={variant.value}
                        onChange={(e) => {
                          const updated = [...formColorVariants];
                          updated[vIdx].value = e.target.value;
                          setFormColorVariants(updated);
                        }}
                        className="w-8 h-8 rounded-none border border-stone-300 cursor-pointer p-0 bg-transparent"
                      ...
                      <input
                        ...
                        className="text-xs py-1 px-2 border border-stone-200 font-sans w-36 sm:w-44 focus:outline-hidden"
                      />
                    </div>

                    {/* Dedicated Color Variant Craft Image URL or Local Upload */}
                    <div className="flex-1 w-full flex items-center gap-2">
                      <div className="w-9 h-9 rounded-none border border-stone-200 shrink-0 overflow-hidden bg-stone-50 flex items-center justify-center relative">
                        {variant.image ? (
                          <CraftImage
                            src={variant.image}
                            alt={variant.name}
                          ...
                          <div
                            ...
                            title="Solid Swatch Color fallback"
                          />
                        )}
                      </div>
                      ...
                      <div className="flex-1 space-y-1">
                        <input
                          ...
                          className="w-full text-[10px] py-1 px-2 border border-stone-200 font-mono focus:outline-hidden bg-white"
                        />
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => {
                            ...
                            if (file) {
                              compressImage(file).then((result) => {
                                const updated = [...formColorVariants];
                                updated[vIdx].image = result;
                                setFormColorVariants(updated);
                              }).catch((err) => alert(err.message || 'Image too large'));
                            }
                          }}
                          className="block w-full text-[8px] text-stone-500 file:mr-2 file:py-0.5 file:px-1.5 file:border file:border-stone-200 file:text-[8px] file:font-sans file:bg-stone-50 hover:file:bg-stone-100 cursor-pointer"
                        />
                      </div>
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const updated = formColorVariants.filter((_, i) => i !== vIdx);
                        setFormColorVariants(updated);
                      }}
                      className="p-1.5 text-stone-400 hover:text-red-500 hover:bg-red-50 transition-colors shrink-0"
          ...
          <div className="flex gap-2 justify-end pt-4 border-t border-[var(--theme-primary)]/10">
            ...
            <button
              ...
              className={`py-2.5 px-5 rounded-none text-white font-sans text-xs font-semibold uppercase tracking-widest transition-colors shadow-xs flex items-center justify-center gap-2 ${
                isSavingProduct 
                  ? 'bg-stone-400 cursor-not-allowed' 
                  : 'bg-[var(--theme-accent)] hover:bg-[var(--theme-primary)]'
              }`}
            >
              {isSavingProduct && <Loader2 className="w-4 h-4 animate-spin" />}
              {isSavingProduct ? 'Uploading...' : 'Save Craft to Catalog'}
            </button>
          </div>
        </form>
      )}
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleSaveSettings} className="space-y-6 bg-white border border-[var(--theme-primary)]/15 p-6 shadow-xs">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-sans uppercase tracking-widest font-semibold text-stone-700 mb-1.5">
                  Hero Showcase Image / Plate (URL or Device Upload)
                </label>
                <input
                  type="text"
                  required
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      compressImage(file).then((result) => {
                        setSettings({ ...settings, heroImageUrl: result });
                      }).catch((err) => alert(err.message || 'Image too large'));
                    }
                  }}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      compressImage(file).then((result) => {
                        setSettings({ ...settings, founderImageUrl: result });
                      }).catch((err) => alert(err.message || 'Image too large'));
                    }
                  }}
                  className="w-full text-xs text-stone-500 file:mr-4 file:py-1.5 file:px-3 file:border file:border-[var(--theme-primary)]/30 file:text-xs file:font-sans file:bg-[var(--theme-bg)] file:text-stone-800 hover:file:bg-[var(--theme-primary)]/10 file:cursor-pointer"
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (file) {
                        alert("Video file uploads are not supported due to database limits. Please use a YouTube link above instead.");
                      }
                    }}
                    className="hidden"
                  />
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        compressImage(file).then((result) => {
                          setSettings({ ...settings, studioTourThumbnailUrl: result });
                        }).catch((err) => alert(err.message || 'Image too large'));
                      }
                    }}
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-sans uppercase tracking-widest font-semibold text-stone-700 mb-1.5">
                  Official Instagram Handle URL
                </label>
                <input
                  type="url"
                  value={settings.instagramUrl || "https://www.instagram.com/vlaksha_crafts?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw=="}
                  onChange={(e) => setSettings({ ...settings, instagramUrl: e.target.value })}
                  placeholder="https://www.instagram.com/vlaksha_crafts..."
                  className="w-full text-xs py-2 px-3 border border-stone-200 focus:outline-hidden focus:border-[var(--theme-accent)] bg-white font-mono text-[11px]"
                />
              </div>
            </div>

            {/* Storewide Discounts & Delivery Fee Controls */}
            <div className="space-y-5 border-t border-stone-200/80 pt-5">
              <h4 className="text-xs font-sans uppercase tracking-[0.2em] text-[var(--theme-primary)] font-bold border-b border-stone-100 pb-2 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <Percent className="w-4 h-4 text-[var(--theme-accent)]" />
                  Storewide Discounts & Delivery Fee Controls
                </span>
                <span className="text-[10px] text-stone-400 font-normal normal-case">Configure customer pricing incentives</span>
              </h4>

              {/* Grid 1: Storewide Discount & Waive Delivery */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-stone-50/80 p-4 border border-stone-200 rounded-xs">
                {/* Storewide Discount Rate */}
                <div className="space-y-2">
                  <label className="block text-[10px] font-sans uppercase tracking-wider font-bold text-stone-800 flex items-center justify-between">
                    <span>Storewide Promotional Discount (%)</span>
                    <span className="text-xs font-mono font-bold text-[var(--theme-primary)] bg-white px-2 py-0.5 border border-stone-200">
                      {settings.storewideDiscountPercent || 0}% OFF
                    </span>
                  </label>
                  <p className="text-[11px] text-stone-500">
                    Automatic storewide discount applied to all client orders at checkout. Set to 0 for no automatic discount.
                  </p>
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      max="50"
                      step="5"
                      value={settings.storewideDiscountPercent || 0}
                      onChange={(e) => setSettings({ ...settings, storewideDiscountPercent: parseInt(e.target.value, 10) })}
                    ...
                    <input
                      ...
                      max="100"
                      value={settings.storewideDiscountPercent || 0}
                      onChange={(e) => setSettings({ ...settings, storewideDiscountPercent: Math.max(0, parseInt(e.target.value, 10) || 0) })}
                      className="w-16 text-xs p-1.5 border border-stone-300 font-mono text-center bg-white font-bold"
                    />
                  </div>
                  {/* Preset Buttons */}
                  <div className="flex gap-1.5 pt-1">
                    {[0, 5, 10, 15, 20, 25].map((rate) => (
                      <button
                        key={rate}
                        type="button"
                        onClick={() => setSettings({ ...settings, storewideDiscountPercent: rate })}
                        className={`px-2 py-1 text-[10px] font-mono cursor-pointer transition-colors ${
                          (settings.storewideDiscountPercent || 0) === rate
                ...
                {/* Delivery Fee & Free Delivery Waiver */}
                <div className="space-y-3 border-t md:border-t-0 md:border-l border-stone-200/80 pt-3 md:pt-0 md:pl-5">
                  <label className="block text-[10px] font-sans uppercase tracking-wider font-bold text-stone-800 flex items-center justify-between">
                    <span className="flex items-center gap-1.5">
                      <Truck className="w-3.5 h-3.5 text-stone-600" />
                      <span>Delivery Shipping Fee Policy</span>
                    </span>
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full font-bold ${
                      settings.isDeliveryWaived || (settings.deliveryFee || 0) === 0
                        ? 'bg-emerald-100 text-emerald-800 border border-emerald-300'
                        : 'bg-amber-100 text-amber-800 border border-amber-300'
                    }`}>
                      {settings.isDeliveryWaived || (settings.deliveryFee || 0) === 0 ? 'FREE DELIVERY (Waived)' : `₹${settings.deliveryFee} Charge`}
                    </span>
                  </label>

                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer select-none bg-white p-2.5 border border-stone-200/80">
                      <input
                        type="checkbox"
                        checked={settings.isDeliveryWaived ?? true}
                        onChange={(e) => setSettings({ ...settings, isDeliveryWaived: e.target.checked })}
                        className="accent-[var(--theme-accent)] w-4 h-4"
                      />
                      <div className="text-xs">
                        <span className="font-bold text-stone-800 block">Waive Delivery Fee Storewide (Free Auspicious Shipping)</span>
                        <span className="text-[10px] text-stone-500 block">When checked, clients pay ₹0 for nationwide courier delivery.</span>
                      </div>
                    </label>

                    {!(settings.isDeliveryWaived ?? true) && (
                      <div className="grid grid-cols-2 gap-3 pt-1">
                        <div>
                          <label className="block text-[9px] font-sans uppercase font-semibold text-stone-600 mb-1">
                          ...
                          <input
                            ...
                            onChange={(e) => setSettings({ ...settings, deliveryFee: Math.max(0, parseInt(e.target.value, 10) || 0) })}
                            className="w-full text-xs p-1.5 border border-stone-300 font-mono bg-white"
                            placeholder="e.g. 99"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setSettings({ ...settings, freeDeliveryThreshold: Math.max(0, parseInt(e.target.value, 10) || 0) })}
                            className="w-full text-xs p-1.5 border border-stone-300 font-mono bg-white"
              ...
              {/* Custom Admin Promo / Voucher Codes Manager */}
              <div className="bg-white p-4 border border-stone-200 rounded-xs space-y-4">
                <div className="flex items-center justify-between border-b border-stone-100 pb-2">
                  <div>
                    <h5 className="text-xs font-sans uppercase tracking-wider font-bold text-stone-900 flex items-center gap-1.5">
                      <Tag className="w-3.5 h-3.5 text-[var(--theme-primary)]" />
                      <span>Custom Admin Promo & Voucher Codes</span>
                    </h5>
                    <p className="text-[11px] text-stone-500">
                      Create promotional codes that clients can enter at checkout to redeem percentage or flat discounts or free shipping.
                    </p>
                  </div>
                  <span className="text-[10px] font-mono text-stone-400 bg-stone-50 px-2 py-0.5 border border-stone-200">
                    Active Codes: {(settings.adminPromoCodes || []).length}
                  </span>
                </div>

                {/* Promo Code Creator Form */}
                <div className="bg-stone-50/80 p-3 border border-stone-200/80 space-y-3">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-stone-700 block">
                    + Add New Custom Promo Voucher Code
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-2.5 text-xs">
                    <div>
                      <label className="block text-[9px] uppercase font-semibold text-stone-500 mb-1">Promo Code</label>
                      <input
                        type="text"
                        value={newPromoCode}
                        onChange={(e) => setNewPromoCode(e.target.value.toUpperCase())}
                        placeholder="e.g. SPECIAL15"
                        className="w-full p-1.5 border border-stone-300 bg-white font-mono uppercase font-bold text-xs"
                      />
                    </div>
                    <div>
                      <label className="block text-[9px] uppercase font-semibold text-stone-500 mb-1">Discount Benefit</label>
                      <select
                        value={newPromoType}
                        onChange={(e) => setNewPromoType(e.target.value as any)}
                        className="w-full p-1.5 border border-stone-300 bg-white text-xs"
                      >
                        <option value="percent">Percentage Off (%)</option>
                        <option value="fixed">Flat Amount Off (₹)</option>
                        <option value="free_shipping">Waive Shipping Fee</option>
                      </select>
                    </div>
                    {newPromoType !== 'free_shipping' && (
                      <div>
                        <label className="block text-[9px] uppercase font-semibold text-stone-500 mb-1">
                          {newPromoType === 'percent' ? 'Discount Rate (%)' : 'Discount Amount (₹)'}
                        </label>
                        <input
                          type="number"
                          min="1"
                          value={newPromoVal}
                          onChange={(e) => setNewPromoVal(Math.max(1, parseInt(e.target.value, 10) || 1))}
                          className="w-full p-1.5 border border-stone-300 bg-white font-mono text-xs"
                        />
                      </div>
                    )}
                    <div>
                      <label className="block text-[9px] uppercase font-semibold text-stone-500 mb-1">Min Order Value (₹)</label>
                      <input
                        type="number"
                        min="0"
                        value={newPromoMinOrder}
                        onChange={(e) => setNewPromoMinOrder(Math.max(0, parseInt(e.target.value, 10) || 0))}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 items-center justify-between">
                    <input
                      ...
                      placeholder="Optional voucher note (e.g. Special Festive Discount for Art Lovers)"
                      className="w-full sm:flex-1 p-1.5 border border-stone-300 bg-white text-xs"
                    />
                    <button
                      type="button"
                      onClick={handleAddPromoCode}
                      disabled={!newPromoCode.trim()}
                      className="w-full sm:w-auto px-4 py-1.5 bg-[var(--theme-accent)] hover:bg-[var(--theme-primary)] disabled:bg-stone-300 text-white font-sans text-xs font-bold uppercase tracking-wider cursor-pointer transition-colors shrink-0"
                    >
                      + Save Promo Code
                    </button>
                  </div>
                ...
                <div className="space-y-2">
                  <span className="text-[10px] uppercase tracking-wider font-semibold text-stone-500 block">
                    Configured Voucher Codes Available at Checkout:
                  </span>
                  {(settings.adminPromoCodes || []).length === 0 ? (
                    <p className="text-xs text-stone-400 italic py-2">No active promo codes configured.</p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {(settings.adminPromoCodes || []).map((promo) => (
                        <div key={promo.code} className="p-2.5 bg-stone-50 border border-stone-200 flex items-center justify-between gap-2 text-xs">
                          <div className="min-w-0 space-y-0.5">
                            <div className="flex items-center gap-1.5">
                              <span className="font-mono font-bold text-stone-900 bg-amber-100 text-amber-900 px-1.5 py-0.5 rounded-none border border-amber-300 text-[11px]">
                                {promo.code}
                              </span>
                              <span className="font-semibold text-emerald-700 font-mono text-[11px]">
                                {promo.discountPercent ? `${promo.discountPercent}% OFF` : promo.discountAmount ? `₹${promo.discountAmount} OFF` : 'Free Shipping'}
                              </span>
                            </div>
                            <p className="text-[10px] text-stone-500 truncate">{promo.description || 'No description'}</p>
                            {promo.minOrderValue ? (
                              <p className="text-[9px] text-stone-400 font-mono">Min order: ₹{promo.minOrderValue}</p>
                            ) : null}
                          </div>
                          <button
                            type="button"
                              ...
                              <label className="py-1.5 px-3 border border-stone-200 hover:border-[var(--theme-primary)] bg-stone-50 hover:bg-[var(--theme-primary)]/10 text-[9px] text-stone-700 font-sans uppercase font-bold tracking-wider cursor-pointer flex items-center justify-center gap-1">
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (file) {
                                      compressImage(file).then((result) => {
                                        const nextImages = [...catImages, result];
                                        const updated = (settings.categories || []).map(c => 
                                          c.id === cat.id ? { 
                                            ...c, 
                                            imageUrls: nextImages, 
                                            imageUrl: c.imageUrl ? c.imageUrl : result 
                                          } : c
                                        );
                                        setSettings({ ...settings, categories: updated });
                                      }).catch((err) => alert(err.message || 'Image too large'));
