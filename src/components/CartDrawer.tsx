...
import { X, Trash2, ShoppingBag, ArrowRight, Sparkles, AlertCircle, Percent, Truck } from 'lucide-react';
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        ...
        >
          ...
          {cart.length > 0 && (() => {
            ...
            const storeDiscountAmount = Math.floor(totalAmount * (storeDiscountPercent / 100));
            const isDeliveryWaived = settings?.isDeliveryWaived ?? true;
            const deliveryFee = isDeliveryWaived ? 0 : (settings?.deliveryFee || 0);
            const netEstimate = Math.max(0, totalAmount - storeDiscountAmount) + deliveryFee;
              ...
              <div className="border-t border-[var(--theme-primary)]/20 p-4 bg-stone-50 space-y-3">
                ...
                <div className="space-y-1 font-sans text-xs">
                  <div className="flex items-center justify-between text-stone-600">
                    ...
                    <span className="font-mono font-medium text-stone-900">₹{totalAmount.toLocaleString('en-IN')}</span>
                  </div>

                  {storeDiscountPercent > 0 && (
                    <div className="flex items-center justify-between text-emerald-700 font-semibold bg-emerald-50 px-2 py-1 border border-emerald-200 text-[11px]">
                      <span className="flex items-center gap-1">
                        <Percent className="w-3 h-3 text-emerald-600" />
                        <span>Admin Studio Discount ({storeDiscountPercent}%)</span>
                      </span>
                      <span className="font-mono">-₹{storeDiscountAmount.toLocaleString('en-IN')}</span>
                    </div>
                  )}

                  <div className="flex items-center justify-between text-[11px]">
                    <span className="text-stone-500 flex items-center gap-1">
                      <Truck className="w-3 h-3 text-[var(--theme-primary)]" />
                      <span>Delivery Shipping Fee</span>
                    </span>
                    <span className="font-mono font-bold text-emerald-700">
                      {isDeliveryWaived || deliveryFee === 0 ? 'FREE (Waived)' : `₹${deliveryFee}`}
                    </span>
                  </div>

                  <div className="flex items-center justify-between border-t border-stone-200 pt-2 text-stone-950 font-bold text-sm">
                    <span className="uppercase tracking-wider font-sans text-[11px]">Estimated Payable</span>
                    <span className="font-mono text-base text-stone-950">₹{netEstimate.toLocaleString('en-IN')}</span>
                  </div>
                </div>

                {/* Delivery Lead Time Notice */}
                <div className="flex gap-2 p-2 rounded-none bg-[var(--theme-primary)]/5 border border-[var(--theme-primary)]/15 text-[10px] text-stone-700">
                ...
                <button
                  onClick={onCheckout}
                  className="w-full py-3.5 rounded-none bg-[var(--theme-accent)] hover:bg-[var(--theme-primary)] text-white font-sans text-xs uppercase tracking-widest font-semibold transition-all shadow-md flex items-center justify-center gap-1.5 cursor-pointer"
                >
