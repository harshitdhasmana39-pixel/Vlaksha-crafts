...
import { Sparkles, ShoppingBag, Package, Truck, CheckCircle, ExternalLink, Calendar, MapPin, Phone, Mail, Edit2, Check, Download, Printer, Loader2, RotateCcw, History, Search, CreditCard, Clock, Filter } from 'lucide-react';
...
export default function UserAccount({ currentUser, onUpdateProfile, onNavigateToCatalog, onReorder }: UserAccountProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'received' | 'in_progress' | 'shipped' | 'delivered'>('all');
  ...
  return (
    <div id="vlaksha-user-account" className="space-y-12 my-6">
      ...
      <div className="lg:col-span-8 space-y-6">
        <div id="order-history" className="bg-white border border-[var(--theme-primary)]/15 p-6 shadow-xs space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-100 pb-4">
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-[var(--theme-accent)]/10 text-[var(--theme-accent)] rounded-xs">
              ...
              <div>
                ...
                <p className="text-[11px] text-stone-500 font-sans">
                  Track all past orders, artisan progress status, and download tax invoices
                </p>
              </div>
            </div>
            <span className="text-xs font-sans font-semibold text-[var(--theme-primary)] bg-stone-50 px-3 py-1 border border-stone-200/80 rounded-full">
          ...
          {orders.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-stone-50/80 p-3.5 border border-stone-200/60 rounded-xs text-xs font-sans">
              <div className="space-y-0.5">
                <span className="text-[10px] text-stone-400 uppercase font-bold tracking-wider">Total Lifetime Spent</span>
                <p className="font-mono font-bold text-stone-900 text-sm text-[var(--theme-primary)]">
                  ₹{orders.reduce((sum, o) => sum + o.totalAmount, 0).toLocaleString('en-IN')}
                </p>
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] text-stone-400 uppercase font-bold tracking-wider">Artisan Progress</span>
                <p className="font-semibold text-stone-800 text-xs flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-amber-600" />
                  <span>
                    {orders.filter(o => o.orderStatus === 'received' || o.orderStatus === 'in_progress').length} In Crafting Phase
                  </span>
                </p>
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] text-stone-400 uppercase font-bold tracking-wider">Latest Order</span>
                <p className="font-medium text-stone-700 text-xs truncate">
                  {orders[0] ? new Date(orders[0].createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-1">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full text-xs pl-8 pr-3 py-2 border border-stone-200 focus:outline-hidden focus:border-[var(--theme-accent)] bg-white rounded-xs"
                />
              </div>
              <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
                <Filter className="w-3.5 h-3.5 text-stone-400 shrink-0 hidden sm:inline" />
                {(['all', 'received', 'in_progress', 'shipped', 'delivered'] as const).map((st) => (
                  <button
                    key={st}
                    onClick={() => setStatusFilter(st)}
                    className={`px-2.5 py-1.5 text-[10px] font-sans uppercase font-medium tracking-wider whitespace-nowrap cursor-pointer rounded-xs transition-colors ${
                      statusFilter === st
          ...
          {loadingOrders ? (
            <div className="bg-white border border-stone-200 p-12 text-center space-y-4 shadow-xs flex flex-col items-center justify-center">
              <Loader2 className="w-10 h-10 text-[var(--theme-primary)] animate-spin" />
              <div className="space-y-1">
                <h4 className="font-serif text-base text-stone-700 font-light">Retrieving Your Order History...</h4>
                <p className="text-[11px] text-stone-400 font-sans max-w-xs mx-auto leading-relaxed">
                  Fetching live order status and hand-painted progress updates from our secure database.
                </p>
              </div>
            </div>
          ) : orders.length === 0 ? (
            <div className="bg-stone-50 border border-dashed border-stone-200 p-12 text-center space-y-4 rounded-xs">
              ...
              <button
                ...
                className="py-2.5 px-6 bg-[var(--theme-accent)] hover:bg-[var(--theme-primary)] text-white font-sans text-[10px] uppercase tracking-widest transition-colors shadow-sm cursor-pointer"
              >
            ...
            <div className="space-y-6">
              {orders
                ...
                .map((order) => (
                  <div key={order.id} className="bg-white border border-[var(--theme-primary)]/20 shadow-xs overflow-hidden rounded-xs">
                    
                    {/* Order header banner */}
                    <div className="bg-stone-50/90 border-b border-[var(--theme-primary)]/10 px-4 py-3 sm:px-6 flex flex-wrap items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <span className="text-[10px] font-sans font-bold uppercase text-stone-400">Order Placed Date</span>
                      ...
                      <div className="space-y-0.5">
                        <span className="text-[10px] font-sans font-bold uppercase text-stone-400">Payment Method</span>
                        <p className="text-xs font-sans text-stone-700 flex items-center gap-1 font-medium">
                      ...
                      <div className="space-y-0.5 text-right">
                        <span className="text-[10px] font-sans font-bold uppercase text-stone-400">Total Amount</span>
                        <p className="text-sm font-mono font-bold text-[var(--theme-primary)]">₹{order.totalAmount.toLocaleString('en-IN')}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 sm:p-6 space-y-4">
                      ...
                      <div className="flex flex-wrap items-center justify-between border-b border-stone-100 pb-4 gap-4">
                        <div className="space-y-1">
                          <span className="text-[10px] uppercase font-sans tracking-wider text-stone-400 block font-semibold">Current Order Status</span>
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-sans font-semibold uppercase tracking-wider bg-[var(--theme-accent)] hover:bg-[var(--theme-primary)] text-white transition-all cursor-pointer shadow-2xs"
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-sans font-medium uppercase tracking-wider border border-stone-200 hover:border-[var(--theme-primary)] text-stone-600 hover:text-[var(--theme-primary)] bg-white transition-colors cursor-pointer"
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-sans font-medium uppercase tracking-wider bg-stone-50 border border-stone-200 hover:bg-[var(--theme-accent)] hover:text-white hover:border-[var(--theme-accent)] text-stone-700 transition-all cursor-pointer"
                            title="Print Receipt"
                          >
                      ...
                      <div className="divide-y divide-stone-100">
                        {order.items.map((item) => (
                          <div key={item.id} className="py-3 flex gap-4 items-center">
                            <div className="w-12 h-12 rounded-full border border-stone-200 overflow-hidden shrink-0 shadow-2xs bg-stone-50">
                              <img
                            ...
                            <div className="text-right text-xs font-mono font-semibold text-stone-800 shrink-0">
                      ...
                      {order.trackingNotes && (
                        <div className="mt-2 p-3 bg-amber-50/60 border-l-3 border-[var(--theme-primary)] text-xs text-stone-700 font-sans space-y-1 rounded-r-xs">
                          ...
                          <p className="italic leading-relaxed text-stone-700">{order.trackingNotes}</p>
                        </div>
                      )}
