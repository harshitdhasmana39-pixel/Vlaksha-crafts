...
export const DEFAULT_SETTINGS: StudioSettings = {
  ...
  instagramUrl: 'https://www.instagram.com/vlaksha_crafts?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==',
  additionalAdmins: '',
  themePreset: 'classic',
  primaryColor: '#c5a059',
  accentColor: '#1a2a4e',
  ...
  storewideDiscountPercent: 10,
  deliveryFee: 0,
  isDeliveryWaived: true,
  freeDeliveryThreshold: 0,
  adminPromoCodes: [
    { code: 'VLAKSHA10', discountPercent: 10, description: '10% Inaugural Artisan Studio Discount' },
    { code: 'FESTIVE20', discountPercent: 20, minOrderValue: 1500, description: 'Festive Season 20% Off on orders above ₹1,500' },
    { code: 'WELCOME100', discountAmount: 100, minOrderValue: 500, description: '₹100 Welcome Gift on orders above ₹500' },
    { code: 'FREESHIP', waiveDelivery: true, description: 'Waive Delivery Shipping Fee completely' }
  ],
  categories: [
    { 
...
// --- IndexedDB Wrapper for Products ---
const IDB_NAME = 'VlakshaDB';
const IDB_VERSION = 1;
...
const setIDB = async (key: string, val: any): Promise<void> => {
  const db = await initDB();
  return new Promise((resolve, reject) => {
...
export const dbService = {
  ...
  async saveProduct(product: Product): Promise<void> {
    ...
    if (product.images) {
      for (let i = 0; i < product.images.length; i++) {
        product.images[i] = await uploadBase64ToStorage(product.images[i], `products/${product.id}/`);
      }
    }
    ...
    if (product.colors) {
      for (let c of product.colors) {
        if (c.image) {
          c.image = await uploadBase64ToStorage(c.image, `products/${product.id}/`);
        }
      }
    }
    ...
    try {
      ...
      // Also save to localStorage as a fallback, but wrap in try-catch in case it's too big
      try {
        localStorage.setItem(PRODUCTS_KEY, jsonString);
      } catch(e) {
  ...
  async addReview(productId: string, userName: string, rating: number, comment: string, photoUrl?: string): Promise<void> {
  ...
  async loginUserWithGoogle(): Promise<{ success: boolean; error?: string; user?: User }> {
    try {
      ...
      if (e.code === 'auth/popup-closed-by-user' || e.code === 'auth/cancelled-popup-request' || e.code === 'auth/unauthorized-domain') {
        console.warn("Firebase Google Auth info/warning:", e);
      } else {
      ...
      if (e.code === 'auth/popup-closed-by-user') {
        ...
        errorMsg = `The domain "${window.location.hostname}" is not authorized for Google Sign-In in Firebase. Please add this domain to the "Authorized domains" list under Firebase Console -> Authentication -> Settings -> Authorized domains.`;
      } else {
        errorMsg = e.message || errorMsg;
      }
      return { success: false, error: errorMsg };
  ...
  getSettings(): StudioSettings {
    ...
    // Recovery for extremely large localstorage items (Quota Exceeded prevention)
    if (data.length > 1000000) { 
      console.warn("Settings payload exceeded 1MB! Scrubbing images to restore functionality.");
      try {
        const parsed = JSON.parse(data);
        if (parsed.founderImageUrl?.startsWith('data:image')) delete parsed.founderImageUrl;
        if (parsed.heroImageUrl?.startsWith('data:image')) delete parsed.heroImageUrl;
        if (parsed.categories) {
          parsed.categories = parsed.categories.map((c: any) => {
            if (c.imageUrl?.startsWith('data:image')) delete c.imageUrl;
            if (c.imageUrls) {
              c.imageUrls = c.imageUrls.filter((u: string) => !u.startsWith('data:image'));
            }
            return c;
          });
        }
  ...
  async saveSettings(settings: StudioSettings): Promise<void> {
    ...
    if (settings.founderImageUrl) settings.founderImageUrl = await uploadBase64ToStorage(settings.founderImageUrl, 'settings/');
    if (settings.heroImageUrl) settings.heroImageUrl = await uploadBase64ToStorage(settings.heroImageUrl, 'settings/');
    if (settings.studioTourThumbnailUrl) settings.studioTourThumbnailUrl = await uploadBase64ToStorage(settings.studioTourThumbnailUrl, 'settings/');
    
    if (settings.categories) {
      for (let c of settings.categories) {
        if (c.imageUrl) c.imageUrl = await uploadBase64ToStorage(c.imageUrl, 'settings/');
        if (c.imageUrls) {
           for (let i = 0; i < c.imageUrls.length; i++) {
             c.imageUrls[i] = await uploadBase64ToStorage(c.imageUrls[i], 'settings/');
           }
        }
      }
    }
    ...
    try {
      ...
      console.log("Settings fully saved to Firestore after storage upload.");
    } catch (err) {
